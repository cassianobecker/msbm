...
def main():
    ...
    model_list_tru_rand = [e for e in model_list if e.startswith('model_tru_rand')]
    for model_file in model_list_tru_rand:

        model_url = os.path.join('models', model_file)
        results_mom, elbo_seq = ut.load_results(model_url)
        # y_rand
        yrand_tru_rand.append(ut.adj_rand(results_mom['MU'], data['Y']))
        # final elbo
        elbo_tru_rand.append(elbo_seq['all'][-1])
    ...
    stats_dict = {
        'yrand_tru_rand': yrand_tru_rand,
        'elbo_tru_rand': elbo_tru_rand,
